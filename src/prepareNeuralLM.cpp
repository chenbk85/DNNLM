...
void writeNgrams(const vector<vector<intern_string> > &data, 
		 ...
		 const string &filename)
{
    ...
    for (int i=0; i<data.size(); i++) {
        vector<string> words(data[i].begin(), data[i].end());
        preprocessWords(words, ngrams, ngram_size, vocab, numberize, add_start_stop, ngramize);
	// write out n-grams
