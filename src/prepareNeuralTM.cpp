...
void writeNgrams(const vector<vector<intern_string> > &data, int source_context_size, int target_context_size, const vocabulary &input_vocab, int source_unk, const vocabulary &output_vocab, bool numberize, const string &filename)
{
...
int main(int argc, char *argv[])
{
    ...
    try
    {
	CmdLine cmd("Prepares training data for training a neural translation model.", ' ', "0.2");
    ...
    intern_string start(string("<s>")), stop(string("</s>"));

    // Read in input training data and validation data
    vector<vector<intern_string> > train_data;
    readSentFile(train_text, train_data);
    
    vector<vector<intern_string> > validation_data;
    if (validation_text != "") {
